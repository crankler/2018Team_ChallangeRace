...
class IsPatrol(Task):
    ...
    def run(self):
        if env.EnemyPoseSave.enemy_dist == 0 or buffiscutted == True:  #看不到敌人或它们加到符了
...
class Random(Task):
    ...
    def run(self):
        ...
        if buffiscutted == True:
            ...
            while 1:
                rospy.sleep(1)
        if rospy.Time.now().secs - self.blackboard.getbufftime1 > 8.5:  # 两轮就跳
            ...
            while np.square(env.MyPose['x'] - goal_x) + np.square(env.MyPose['y'] - goal_y) > 0.03 or np.abs(
                ...
                rospy.sleep(0.01)
                if rospy.Time.now().secs - time2 > 5:
                    print 'time is out 5555555555555555555555555555555'
                    break

            time2 = rospy.Time.now().secs
            while rospy.Time.now().secs - time2 < 3.5:  #抢符时间
